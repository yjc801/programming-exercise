def canJump(A):
    step = 0
    return helper(A,step)

def helper(A,step):
    if step >= len(A)-1:
        return True

    ret = False
    for i in xrange(A[step],0,-1):
        ret |= helper(A,step+i)
    return ret

def canJump2(A):
	max_jump = 1
	i = 0
	while i < max_jump < len(A):
		max_jump = max(max_jump,i+A[i]+1)
		i+=1
	return max_jump >= len(A)

if __name__ == '__main__':
	print canJump2([0])
	print canJump2([2,3,0,1,4])
	print canJump2([2,3,1,1,4])
	print canJump2([3,2,1,0,4])
	print canJump2(range(25000,0))
	print canJump2([2,0,6,9,8,4,5,0,8,9,1,2,9,6,8,8,0,6,3,1,2,2,1,2,6,5,3,1,2,2,6,4,2,4,3,0,0,0,3,8,2,4,0,1,2,0,1,4,6,5,8,0,7,9,3,4,6,6,5,8,9,3,4,3,7,0,4,9,0,9,8,4,3,0,7,7,1,9,1,9,4,9,0,1,9,5,7,7,1,5,8,2,8,2,6,8,2,2,7,5,1,7,9,6])